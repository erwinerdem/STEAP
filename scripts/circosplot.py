...
def get_gwas_links(
    ...
    """
    Finds all correlations between the input gwas_name and gwas part of the pattern.
    """
...
def preprocessing(
    ...
    """
    Return links, nodes and gwas_links. The links (correlations) and nodes are used for the inner chord plot.
    The gwas links are used for the annotation layers.
    """
...
def plot(
    ...
    """
    Returns a pandas dataframe showing which cell types are shared in which gwas phenotype group.

    Parameters
    ----------
    corr_df : pd.DataFrame
        Pandas dataframe containing the correlation and pvalue between the gwas phenotypes.
    gwas_group_dict : dict[str, list[str]]
        Dictionary with the phenotype group name as key and (regex) keywords
        of the phenotypes in a list as values. The values will be used as nodes.
    gwas_name : str or None
        The name of the gwas used in the input dataframe which is to be plotted in the outer circles
        as bar plots (annotation layer). If set to None the annotation layers won't be generated.
    sign_threshold : int
    ...
    bottom : float
        The position (radius in polar coordinates) where the nodes will be plotted.
    ylim : float
        Set the ylim of the plot (radius in polar coordinates).
    color_map : str
        The colormap used for the nodes. The nodes will be colorcoded based on which
        key the belong to in the gwas_group_dict.
    ...
    links.apply(lambda row: chord_plot_pandas(gcircle, row, gwas_group_dict, corr_limit, bottom), axis=1)
